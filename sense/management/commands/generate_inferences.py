#!/usr/bin/python
import datetime
import os
import random
import re
import sys
import time
import csv
from optparse import make_option
from collections import namedtuple

#http://pythonadventures.wordpress.com/tag/ascii/
reload(sys)
sys.setdefaultencoding("utf-8")

from django.conf import settings
from django.core.management.base import BaseCommand
from django.core import serializers
from django.db.models import F, Q
import django
from django.utils.encoding import smart_text, smart_unicode

from sense import models
from sense import constants as c
from sense import settings as _settings

class Command(BaseCommand):
    help = 'Generate inferred triples.'
    args = ''
    option_list = BaseCommand.option_list + (
        make_option('--dryrun',
            action='store_true',
            dest='dryrun',
            default=False,
            help='If given, does not commit any changes.'),
        make_option('--predicate',
            dest='predicate',
            default=None,
            help='The predicate to filter generation by.'),
        make_option('--subject',
            dest='subject',
            default=None,
            help='The subject to start at.'),
        make_option('--object',
            dest='object',
            default=None,
            help='The object to start at.'),
        make_option('--context',
            dest='context',
            default='global',
            help='The context to limit inference to.'),
        make_option('--limit',
            dest='limit',
            default=0,
            help='The total number of inferences to make.'),
        )
    
    def handle(self, *args, **options):
        context = models.Context.objects.get(name=options['context'])
        while 1:
            ret = context.infer(limit=int(options['limit']))
            print 'ret:',ret
            if not ret:
                print
                print 'Done!'
                return
        